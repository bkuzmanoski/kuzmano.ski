 ...
 *
 * Uses the native `dblclick` event for mouse input and pairs pointer releases for touch and pen input,
 * including browsers that do not synthesize `dblclick` for double taps.
 *
 * Two presses must occur within `DOUBLE_PRESS_INTERVAL` and within `TOLERANCE` pixels of each other. A
 * press that travels beyond `TOLERANCE` is treated as a drag and cannot participate in a double press.
 *
 * Drag handling beyond this gesture-level check remains the caller's responsibility.
 */
