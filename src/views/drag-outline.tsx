 ...
 *
 * A move shows the shape of the window alone. A resize also shows the borders that divide its
 * chrome, which is how the size being chosen is read: the title bar keeps its height while the
 * pane below it grows, and the scrollbar keeps its width. Both are drawn unconditionally
 * because a resizable window always shows them. The resize control lives in the scrollbar,
 * which is why the scrollbar cannot collapse while one is present (see `scrollbar.tsx`), and
 * a maximized window offers no resize control at all.
 *
