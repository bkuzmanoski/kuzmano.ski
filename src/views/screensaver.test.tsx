...
test("a finger dragging across it does not, so the click that ends a tap is caught by the screensaver", () => {
  const screensaver = raise();
...
test("input that trails the gesture it was raised with does not dismiss it", () => {
  const { container } = render(<Screensaver />);
